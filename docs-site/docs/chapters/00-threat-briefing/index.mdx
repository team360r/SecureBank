...
**Fort Knox** is a deliberately vulnerable banking app. Your job across this tutorial is to find every weakness and seal it shut.
...
```bash title="Terminal"
git clone https://github.com/example/fort-knox-flutter.git
cd fort-knox-flutter
...
You should see a login screen for **FortKnox Banking**. The app lets you:
...
class AuthService {
  // highlight-next-line
  static const String _apiKey = 'sk_live_fortknox_9a8b7c6d5e4f3g2h1i';

  Future<bool> login(String email, String password) async {
    // highlight-next-line
    print('Login attempt: email=$email, password=$password');

    if (email == 'admin@fortknox.co.uk' && password == 'password123') {
      // highlight-next-line
