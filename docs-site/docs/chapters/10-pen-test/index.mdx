...
You have spent nine chapters hardening SecureBank. But what happens when a new developer joins the team and adds a `print(password)` call? Or when a dependency with a known CVE gets pulled in? Right now, nothing stops them. There are no automated gates.
...
For SecureBank-specific rules that go beyond built-in lints, create a custom lint package using `custom_lint`:
...
PluginBase createPlugin() => _SecureBankLints();

class _SecureBankLints extends PluginBase {
  @override
...
:::info Checkpoint
At this point you have strict static analysis that catches security anti-patterns at compile time, custom lint rules specific to SecureBank, and a dependency auditing script. In Part 2, you will wire these into a GitHub Actions pipeline so no vulnerable code can reach the main branch.
:::
