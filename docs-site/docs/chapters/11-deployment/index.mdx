...
> *"A castle's gate is strongest when the drawbridge is raised and the portcullis is down. A mobile app is strongest when every secret is sealed and every build is signed."* — SecureBank field manual
...
Open the SecureBank starter project and look at how it handles configuration:
...
```bash title="Terminal"
flutter run \
  --dart-define=API_KEY=sk_live_securebank_9a8b7c6d5e4f3g2h1i \
  --dart-define=API_BASE_URL=https://api.securebank.co.uk/v1 \
...
API_KEY=sk_test_securebank_dev_key_123
...
keytool -genkey -v \
  -keystore securebank-release.jks \
...
keyAlias=securebank
storeFile=/path/to/securebank-release.jks
```
  ...
  build-release:
    ...
    steps:
      ...
      - name: Decode signing keystore
        run: echo "${{ secrets.ANDROID_KEYSTORE_BASE64 }}" | base64 -d > android/securebank-release.jks
      ...
      - name: Create key.properties
        run: |
          ...
          keyAlias=securebank
          storeFile=securebank-release.jks
          EOF
```
