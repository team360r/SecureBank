...
```mermaid
sequenceDiagram
    participant App as FortKnox App
...
class ApiConfig {
  ...
  static const String baseUrl = 'https://api.fortknox.co.uk';
...
When your app connects to `https://api.fortknox.co.uk`, the server presents a TLS certificate. By default, the OS verifies this certificate against its trust store of Certificate Authorities (CAs). The problem is that the trust store can be modified:
