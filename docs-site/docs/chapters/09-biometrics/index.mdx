...
Open the SecureBank app, log in, then switch to another app. Switch back. You are still on the dashboard — no challenge, no lock screen, full access to the user's balance and transfer capability.
...
class DashboardScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // highlight-end
    return Scaffold(
      appBar: AppBar(title: const Text('SecureBank')),
      body: Column(
        children: [
...
<dict>
    ...
    <string>SecureBank uses Face ID to verify your identity before showing account details.</string>
    <!-- highlight-end -->
</dict>
```
...
Create a service that wraps `local_auth` with SecureBank-specific logic:
...
class BiometricService {
  ...
  Future<bool> authenticate({
    String reason = 'Verify your identity to access SecureBank',
  }) async {
    try {
...
class _BiometricGateState extends State<BiometricGate>
  ...
  Future<void> _lockAndChallenge() async {
    ...
    final authenticated = await widget.biometricService.authenticate(
      reason: 'Verify your identity to resume SecureBank',
    );
...
void main() {
  group('BiometricGate', () {
    ...
    testWidgets('shows child when not locked', (tester) async {
      ...
      expect(find.text('SecureBank Locked'), findsNothing);
    });
