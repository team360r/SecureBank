...
SecureBank has three user roles: `customer`, `teller`, and `admin`. Each role should see different screens. But right now, there is nothing enforcing that. Look at the router configuration:
...
```
securebank://app/admin/users
```

If your app registers this scheme, the OS opens SecureBank and navigates straight to `/admin/users`. Without a route guard, the admin panel is exposed to anyone who knows the path.
...
Launch SecureBank, log in as a `customer`, and then trigger a deep link:

```bash title="Testing on Android emulator"
adb shell am start -a android.intent.action.VIEW \
  -d "securebank://app/admin/audit-log" \
  com.securebank.app
```
...
You have closed the open-corridor vulnerability in SecureBank. Every route now requires authentication by default, and role-sensitive routes demand the correct `UserRole` before granting access. Deep links are funnelled through the same redirect guard as normal navigation, so there is no side door.
